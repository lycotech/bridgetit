  ...
  return (
    <section className="relative overflow-hidden border-t border-border py-20 md:py-32">
      <div
        ...
        style={{ background: "radial-gradient(ellipse 70% 60% at 50% 42%, hsl(var(--primary) / 0.10), transparent)" }}
        aria-hidden
      />
      {/* Teal holds the argument at the top; gold gathers under the closing
          lockup, so the section itself travels from payroll to prosperity. */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: "radial-gradient(ellipse 55% 45% at 50% 96%, hsl(var(--gold) / 0.12), transparent)" }}
        aria-hidden
      />
      <div className="relative mx-auto max-w-4xl px-5 text-center md:px-8">
        ...
        <Reveal delay={0.15}>
          {/* The closing beat of the page — the one place the full lockup,
              tagline and all, is allowed to take up room. It is sized large
              deliberately: the tagline is set off the wordmark, so anything
              under about 96px turns "from payroll to prosperity" into a
              smudge. */}
          <div className="mt-12 flex flex-col items-center gap-4">
            <Logo className="h-24 md:h-32" withTagline />

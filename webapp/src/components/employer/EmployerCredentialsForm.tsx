...
/**
 * The company email/password (+ optional TOTP) form on its own, with no
 * surrounding page — the inline counterpart to the full-page sign-in at
 * /employer-portal/login. Extracted for the same reason
 * `AdminCredentialsForm` is: one real form used in two places, so a future
 * change to the sign-in flow cannot silently miss the inline entrance.
 */
export function EmployerCredentialsForm({ idPrefix = "employer" }: { idPrefix?: string }) {

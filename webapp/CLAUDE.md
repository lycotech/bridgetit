...
<backend>
  ...
  In production (Vercel), vercel.json rewrites /api/* to the Render backend URL, so relative
  URLs keep working there too — the browser only ever talks to the Vercel domain, which is
  what keeps session/CSRF cookies working across the two separate hosts.

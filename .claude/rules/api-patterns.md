...
<prisma_strategy>
  This app runs Postgres (Neon) via DATABASE_URL/DIRECT_URL, not SQLite.
  Dev/preview: npx prisma migrate dev (creates/applies a migration)
  Production: npx prisma migrate deploy (see backend/render.yaml) — never db push against production data.

  Startup issue: If routes return 404 but health works, Prisma client wasn't generated.
  Fix: npx prisma generate && restart backend
</prisma_strategy>

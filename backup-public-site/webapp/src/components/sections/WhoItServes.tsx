import { Users, Building2, Landmark, Boxes } from "lucide-react";
    ...
    >
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        <Reveal delay={0.1} className="lg:hidden">
          ...
          >
            {AUDIENCES.map(({ icon: Icon, tag, title, body }, i) => (
              <AccordionItem
                key={tag}
                value={`item-${i}`}
        ...
        <div className="relative mx-auto mt-14 hidden max-w-4xl lg:block">
          ...
          />
          {/* central hub */}
          <span className="absolute left-1/2 top-1/2 z-10 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-primary/40 bg-card text-primary ring-8 ring-background">
            <LogoMark className="h-6" />
          </span>

          <StaggerGroup className="grid grid-cols-2 gap-6">
            {AUDIENCES.map(({ icon: Icon, tag, title, body }) => (
              <MDiv
                key={tag}
